...
@app.route('/is_admin')
def is_admin():
    return database.query_database(database=USER_DB, query='SELECT isAdmin FROM users WHERE username = ?', parameters=(username))
...
def login():
    ...
    if request.method == 'POST':
        ...
        result = database.query_database(database=USER_DB, query='SELECT * FROM users WHERE username = ? AND password = ?', parameters=(l_username, l_password))
...
def signup():
    ...
    if request.method == "POST":
        ...
        result = database.query_database(database=USER_DB, query='SELECT * FROM users WHERE username = ?', parameters=(l_username,))
        if not result:  # If no result is found, the username is available
            database.query_database(database=USER_DB, query='INSERT INTO users (username, password, firstName, lastName, email, isAdmin) VALUES (?, ?, ?, ?, ?, ?)', parameters=(l_username, l_password, l_firstName, l_lastName, l_email, False))
            username = l_username
            return redirect(url_for('hello'))
        else:
...
@app.route("/users")
def users():
    user_list = database.query_database(database=USER_DB, query='SELECT username, password, firstName, lastName, email FROM users')
